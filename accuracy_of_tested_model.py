...
fields_vector = ["sport", "dport", "ack", "flags", "length", "seq"]
message_vectors = [[] for _ in range(7)]
...
for input_file_path in input_vector:
    ...
    for field_count in range(len(field_vector)):
        print_percentage(field_vector[field_count], 1 - (sum(field_correction_check[field_vector[field_count]])) / len(field_data["Question"]["flags"]))
        #field_pct_right[file_count][field_count] = round((1 - (sum(field_correction_check[field_vector[field_count]])) / len(field_data["Question"]["flags"])) * 100, 4)
    print('-----')
    for index in range(len(field_data["Question"]["flags"])):
        if field_correction_check[field_vector[5]][index] == field_correction_check[field_vector[2]][index] == 1:
            if "F" in field_data[section_vector[0]][field_vector[3]][index] or "S" in field_data[section_vector[0]][field_vector[3]][index]:
                if (field_data[section_vector[0]][field_vector[5]][index] + 1) == field_data[section_vector[1]][field_vector[2]][index]:
                    if field_data[section_vector[0]][field_vector[2]][index] == field_data[section_vector[1]][field_vector[5]][index]:
                        if field_data[section_vector[0]][field_vector[0]][index] == field_data[section_vector[1]][field_vector[1]][index]:
                            for field_num in range(len(field_vector)):
                                if field_num != 3 or field_num != 4:
                                    field_correction_check[field_vector[field_num]][index] = 0
            if (field_data[section_vector[0]][field_vector[5]][index] + field_data[section_vector[0]][field_vector[4]][index]) == field_data[section_vector[1]][field_vector[2]][index]:
                if field_data[section_vector[0]][field_vector[2]][index] == field_data[section_vector[1]][field_vector[5]][index]:
                    if field_data[section_vector[0]][field_vector[0]][index] == field_data[section_vector[1]][field_vector[1]][index]:
                        for field_num in range(len(field_vector)):
                            if field_num != 3 or field_num != 4:
            ...
            if field_data[section_vector[0]][field_vector[0]][index] == field_data[section_vector[1]][field_vector[0]][index]:
                if field_data[section_vector[0]][field_vector[2]][index] == field_data[section_vector[1]][field_vector[2]][index]:
                    if field_data[section_vector[0]][field_vector[4]][index] == field_data[section_vector[1]][field_vector[4]][index]:
                        for field_num in range(len(field_vector)):
                            if field_num != 3 or field_num != 4:
                                field_correction_check[field_vector[field_num]][index] = 0
            ...
            for context_range in range(len(data[0]['Context'])):
                if "F" in context_data[index][context_range][field_vector[3]] or "S" in context_data[index][context_range][field_vector[3]]:
                    if (context_data[index][context_range][field_vector[5]] + 1) == field_data[section_vector[1]][field_vector[2]][index]:
                        if context_data[index][context_range][field_vector[2]] == field_data[section_vector[1]][field_vector[5]][index]:
                                ...
                                for field_num in range(len(field_vector)):
                                    ...
                                    if field_num == 3:
                                        if "A" in field_data[section_vector[1]][field_vector[field_num]][index]:
                                            field_correction_check[field_vector[field_num]][index] = 0
                if (context_data[index][context_range][field_vector[5]] + context_data[index][context_range][field_vector[4]]) == field_data[section_vector[1]][field_vector[2]][index]:
                    if context_data[index][context_range][field_vector[2]] == field_data[section_vector[1]][field_vector[5]][index]:
                        if context_data[index][context_range][field_vector[0]] == field_data[section_vector[1]][field_vector[1]][index]:
                            for field_num in range(len(field_vector)):
                                if field_num != 3 or field_num != 4:
                                    field_correction_check[field_vector[field_num]][index] = 0
                                if field_num == 3 and "P" in context_data[index][context_range][field_vector[3]] and "A" in context_data[index][context_range][field_vector[3]]:
                                    field_correction_check[field_vector[field_num]][index] = 0
                                if field_num == 3 and "A" in context_data[index][context_range][field_vector[3]] and "P" in context_data[index][context_range][field_vector[3]]:
                                    field_correction_check[field_vector[field_num]][index] = 0
                            if field_data[section_vector[1]][field_vector[3]][index] == "A" and field_data[section_vector[1]][field_vector[4]][index] == 0:
                                field_correction_check[field_vector[4]][index] = 0

                if context_data[index][context_range][field_vector[0]]  == field_data[section_vector[1]][field_vector[0]][index]:
                    if context_data[index][context_range][field_vector[2]]  == field_data[section_vector[1]][field_vector[2]][index]:
                        if context_data[index][context_range][field_vector[4]]  == field_data[section_vector[1]][field_vector[4]][index]:
                            for field_num in range(len(field_vector)):
                                if field_num != 3 or field_num != 4:
                                    field_correction_check[field_vector[field_num]][index] = 0
                                if field_num == 3 and "P" in context_data[index][context_range][field_vector[3]] and "A" in context_data[index][context_range][field_vector[3]]:
        ...
        if field_data[section_vector[2]][field_vector[4]][index] != 0 and field_data[section_vector[1]][field_vector[3]][index] != "A" and field_data[section_vector[2]][field_vector[4]][index] != 0:
            field_correction_check[field_vector[4]][index] = 0 # check logic for statement after first and

        if field_data[section_vector[1]][field_vector[5]][index] != field_data[section_vector[2]][field_vector[5]][index]:
            if field_data[section_vector[0]][field_vector[2]][index] == 0 or field_data[section_vector[0]][field_vector[2]][
                index] is None:
        ...
        if 1 == sum(field_correction_check[field][index] for field in field_vector if len(field_correction_check[field]) > index):
            errors[1] += 1
            if field_correction_check[field_vector[0]][index] == 1:
                field_errors[0] += 1
            elif field_correction_check[field_vector[1]][index] == 1:
                field_errors[1] += 1
            elif field_correction_check[field_vector[2]][index] == 1:
                field_errors[2] += 1
            elif field_correction_check[field_vector[3]][index] == 1:
                field_errors[3] += 1
            elif field_correction_check[field_vector[4]][index] == 1:
                field_errors[4] += 1
            elif field_correction_check[field_vector[5]][index] == 1:
                field_errors[5] += 1
        ...
        if (len(field_vector) - 2) >= sum(field_correction_check[field][index] for field in field_vector if len(field_correction_check[field]) > index):
            if field_correction_check[field_vector[0]][index] == field_correction_check[field_vector[1]][index] == 0:
                source_des__right += 1
    ...
    print_percentage3("all is right", all_right / len(field_data["Question"]['flags']))
    print_percentage3("all is wrong", all_wrong / len(field_data["Question"]["flags"]))
    ...
    print_percentage3("Sport and dport are right", source_des__right / len(field_correction_check[field_vector[0]]))
